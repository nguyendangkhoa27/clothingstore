	...
	public ProductDTO update(ProductDTO productDTO) {
		try {
				if(productDTO!=null) {
					...
					newProduct.setCategory(categoryConvert.toEntity(categoryService.findById(productDTO.getCategorySlug().getId())));
						...
						if(oldProduct!=null) {
								...
								for(EntityAmount amount : oldProduct.getAmounts()) {
									...
									for(AmountShort amountShort : productDTO.getInfoProduct()) {
										if(amount.getColor().getId() == amountShort.getColor().getId() && amount.getSize().getId() == amountShort.getSize().getId() ) {
											check = false;
